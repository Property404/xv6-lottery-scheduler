...
sbrktest(void)
{
  ...
  p = sbrk(amt);
  if(p != a){
    fprintf(stdout, "sbrk test failed 640K test, p %x a %x\n", p, a);
    exit();
  }
  ...
  *lastaddr = 99;
  ...
  if(c != (char*)0xffffffff){
    fprintf(stdout, "sbrk allocated more than 640K, c %x\n", c);
    exit();
  }
  ...
  c = sbrk(4096);
  if(c != (char*)0xffffffff){
    fprintf(stdout, "sbrk was able to re-allocate beyond 640K, c %x\n", c);
    exit();
  }
